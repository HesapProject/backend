...
public class CustomProductRequestRepositoryImpl implements CustomProductRequestRepository {

  private final R2dbcEntityTemplate template;

  // product_requests buyer_in/seller_in/requester_in/contract_id'ni to'g'ridan-to'g'ri saqlaydi — JOIN'siz.
  private static final String BASE =
      "SELECT * FROM document.product_requests c WHERE c.deleted = false";

  @Override
  public Flux<ProductRequestEntity> findFiltered(
      String buyerIn,
      String sellerIn,
      String fromIn,
      String toIn,
    ...
    return spec.map((row, meta) -> template.getConverter().read(ProductRequestEntity.class, row, meta))
        .all();
  }
}
