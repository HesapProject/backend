...
public class CustomClaimRepositoryImpl implements CustomClaimRepository {
  ...
  // claims endi from_in/to_in (PINFL) to'g'ridan-to'g'ri saqlaydi — JOIN kerak emas.
  private static final String BASE = "SELECT %s FROM document.claims c WHERE c.deleted = false";

  private static String where(String fromIn, String toIn) {
    StringBuilder sb = new StringBuilder();
    if (fromIn != null && !fromIn.isBlank()) sb.append(" AND c.from_in = :fromIn");
    if (toIn != null && !toIn.isBlank()) sb.append(" AND c.to_in = :toIn");
    return sb.toString();
  }
  ...
  @Override
  public Flux<ClaimEntity> findFiltered(String fromIn, String toIn, int size, long offset) {
    String sql =
    ...
    DatabaseClient.GenericExecuteSpec spec =
        bind(template.getDatabaseClient().sql(sql), fromIn, toIn)
            .bind("size", size)
            .bind("offset", offset);
    return spec.map((row, meta) -> template.getConverter().read(ClaimEntity.class, row, meta))
        .all();
  }
