...
public class JsonToListConverter<T> implements Converter<Json, List<T>> {
  ...
  private static final Logger logger = LogManager.getLogger("JsonToListConverter");

  private final ObjectMapper objectMapper;

  public JsonToListConverter(Jackson2ObjectMapperBuilder objectMapper) {
    this.objectMapper = objectMapper.build();
  }

  @Override
  public List<T> convert(Json json) {
    try {
