#!/usr/bin/env bash
# x100 ortiqcha ko'paytirilgan shartnoma narxlarini tuzatish. [apply] bilan yozadi.
set -uo pipefail
APPLY=false
...
docker run --rm -i --network hesap-postgres -v "$CI_PROJECT_DIR/scripts/migration":/sql \
  ...
  psql -h postgres -p 5435 -U "$NU" -d hesap -v ON_ERROR_STOP=1 -v apply="$APPLY" -f /sql/12-fix-price-x100.sql
